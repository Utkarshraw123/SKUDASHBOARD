...
- **Edit / Void / part-code filters (added 2026-07-22, design `docs/superpowers/specs/2026-07-22-goods-in-edit-void-filters-design.md`):**
  - **Part-code filters** on BOTH the tasks list and the filed-records list (independent selection each): All / Bulk (1) / RMs (2) / Finished Goods (3) / Ancillaries (4) / Other. Derived from the LEADING DIGIT of the part number via `partCategory()` in `lib/goods-in.ts` (NOT the free-text `partType` PO column). Tasks table row key now includes the row index (`t.po+t.partNumber+i`) — multi-line POs repeat po+part, and the old key stranded duplicate-key rows in the DOM when filtering.
  - **Record ID column (col R, index 17)** added to the Goods In tab → **18 columns A–R now** (`GOODS_IN_HEADERS`; reads `A2:R2000`). New records get `${po}-${part}-${Date.now()}`; the legacy row (blank Record ID) is targeted by fallback key `${po} ${part} ${timestamp}`.
  - **Edit a filed record in place:** "Filed · reopen" pre-fills the form from the saved record; saving with an `editMode` flag calls `updateGoodsInRecord(recordId, row, fallbackKey)` (overwrites `A{n}:R{n}`) instead of appending — no duplicate. A legacy edit also upgrades that row with a fresh Record ID. **Edit mode is detected by an explicit `editMode="1"` form field, NOT by recordId truthiness** — the real PO2600151 record has a blank Record ID, and gating on recordId duplicated it (fixed).
  - **Delete = Void:** `voidGoodsInRecord()` sets the row's Status cell (col Q) to `"Void"`; `parseGoodsInRecords` drops Void rows, so the PO line returns to the awaiting "G-In form" list. Audit trail preserved (nothing erased). Password-gated `POST /api/goods-in/void`.
  - Known minors (non-blocking): void route returns 500 (not 404) on no-match; update/void don't re-label the header row (cosmetic — reads are positional; header self-heals on the next appended record).
- **Multi-line PO grouping (added 2026-07-23, spec `docs/superpowers/specs/2026-07-22-goods-in-multiline-po-design.md`):**
  - The Goods In tasks list now GROUPS open POs by PO number (`buildGoodsInPoTasks` → `GoodsInPoTask`/`GoodsInLine`; `summarisePo`). One row per PO with an "X of N filed" badge and status **awaiting / partial / filed** (partial = some lines filed). Urgency/earliest-due comes from the earliest UNFILED line.
  - **Stock-only:** only lines with `partCategory` 1/2/3/4 appear (`isStockPart`); freight/service/`ZZ…`/5-/7-codes are excluded, and the **"Other" filter chip is removed** (`GOODS_IN_PART_CHIPS`).
  - **Keep both forms:** a single-line PO opens the original `GoodsInForm` modal (`toSingleTask` reconstructs a `GoodsInTask`); a multi-line PO opens **`GoodsInPoForm`** — a table of all products auto-populated (Part/Desc/Qty read-only), per-line Supplier Code/Batch-Lot/BBD, a "Received" tick that AUTO-TICKS when a Batch/Lot is typed. Partial deliveries: only ticked lines are saved; the rest stay awaiting.
  - **Batch save** `POST /api/goods-in/po` (`poLinesToRecords` + shared `uploadGoodsInAttachments`): loops the submitted lines doing update-in-place (existing) or append (new), then `revalidateTag("sheets")` in a `finally`. The form only submits **NEW or CHANGED** lines and **preserves stored CofA/doc links** on edited existing lines (`existingCoa`/`existingDocs` → `coaUrl: input.coaUrl || l.existingCoa`) — do NOT reintroduce blanket rewrites (that wiped attachments; fixed 2026-07-23).
  - **Combined Word doc** `buildGoodsInPoDoc` (`POST /api/goods-in/doc/po`): one QA13-CF01 for the PO with a products table (Part/Desc/Qty/Supplier Code/Batch-Lot/BBD) + the checklist/sign sections once. Single-line `buildGoodsInDoc` unchanged. Filed records now group by PO with a **"Word (all)"** download.
  - One CofA + docs set per delivery, applied to the lines saved in that submission (unchanged Vercel-Blob dependency).
